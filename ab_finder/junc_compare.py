...
parser = argparse.ArgumentParser("For a MongoDB collection, plots the germline divergence against the junctional sequence identity to a given 'subject' sequence.")
...
parser.add_argument('-x', '--chain', dest='chain', default='heavy', choices=['heavy', 'kappa', 'lambda', 'light'], 
...
def get_sequences(collection, temp_dir, log):
	...
	for seq in query_results:
		fastas.append('>{}_{}\n{}'.format(seq['seq_id'], seq['aa_identity']['v'], seq['junc_aa']))
		seq_counter += 1
		total_seq_counter += 1
...
def query(collection):
	...
	print_query_info()
	return coll.find({'chain': {'$in': chain}},{'_id': 0, 'seq_id': 1, 'aa_identity.v': 1, 'junc_aa': 1})
...
def update(collection, data, standard):
	...
	score = data[0]
	ids = data[1]
	coll.update({'seq_id': {'$in': ids}}, {'$set': {'junc_identity.{}'.format(standard.lower()): float(score)}}, multi=True)
...
def _run_jobs_via_multiprocessing(files, standard, log):
	...
	if args.debug:
		for f in files:
			results.extend(identity(f, standard))
	else:
		p = mp.Pool()
		async_results = []
		for f in files:
			async_results.append(p.apply_async(identity, (f, standard)))
...
def _run_jobs_via_celery(files, standard, log):
	...
	async_results = []
	for f in files:
		async_results.append(run_vdj.delay(f, standard))
